...
import { provideDeleteMutationFn, provideListQueryFn, queryClient } from '../../../hook/use-query';
...
const PetListComponent: Component = () => {
  ...
  createEffect(() => {
    if (!petListQuery.data) {
      return;
    }

    const query = getQuery();
    const maxPage = Math.ceil(petListQuery.data.count / limit);

    if (query.page > maxPage) {
      navigate(`/pet?${qs.stringify({ ...query, page: maxPage })}`);
    }
  });
  ...
  return (
    <Show when={!petListQuery.isLoading}>
      <div data-testid="page-pet-list">
        <Show when={petListQuery.error}>{(getHttpError) => <HttpErrorPartial httpError={getHttpError()} />}</Show>
        <H1>{pageTitle}</H1>
